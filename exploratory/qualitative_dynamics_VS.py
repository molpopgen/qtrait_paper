...
#This script varies VS while holding fitness|effect size constant,
#which also serves to hold mutational variance constant.
#Also illustrates how to keep optimum shift on the correct scale.
...
popstats_output = pd.HDFStore('popstats_vary_VS.h5','w',complevel=6,complib='zlib')
sfs_output = pd.HDFStore('sfs_vary_VS.h5','w',complevel=6,complib='zlib')

for vs in sorted(VS):
    mu = reference_vg/(4.0*vs)
    #sigma = math.sqrt(reference_vm/(2.0*mu))
    sigma=reference_sigma*math.sqrt(vs)
