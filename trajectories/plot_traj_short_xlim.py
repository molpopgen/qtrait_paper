...
popstatFiles = ['../sqlite/H2_1.0_OPT_1_mu_0.005_sigmu0.25_stats.db',
                '../sqlite/H2_1.0_OPT_1_mu_0.001_sigmu0.25_stats.db', '../sqlite/H2_1.0_OPT_1_mu_0.00025_sigmu0.25_stats.db']
trajFiles = ['H2_1.0_OPT_1_mu_0.005_sigmu0.25_traj_merged.db',
             'H2_1.0_OPT_1_mu_0.001_sigmu0.25_traj_merged.db', 'H2_1.0_OPT_1_mu_0.00025_sigmu0.25_traj_merged.db']
oFiles = ['trajectories_himu.pdf',
          'trajectories_medmu.pdf', 'trajectories_lomu.pdf']
...
# Row titles
_T3.set_title(r'$\mu = 0.005, z_0 = 1, \hat\gamma = $' +
              '{0:0.3f}'.format(2.0 * math.sqrt(2) * math.sqrt(0.005)))
_T2.set_title(r'$\mu = 0.001, z_0 = 1, \hat\gamma = $' +
              '{0:0.3f}'.format(2.0 * math.sqrt(2) * math.sqrt(0.001)))
_T1.set_title(r'$\mu = 2.5\times 10^{-4}, z_0 = 1, \hat\gamma = $' +
              '{0:0.3f}'.format(2.0 * math.sqrt(2) * math.sqrt(0.00025)))
...
for statfile, trajfile, mu in zip(reversed(popstatFiles), reversed(trajFiles), reversed(muvals)):
    ...
    pstats = pd.read_sql(
        'select * from data where rep == 42 and generation >= 45000 and generation <= 50250', con)
    con.close()
    pstats['scaled_time'] = pd.Series(
        (pstats.generation - 50000), dtype=np.float64) / 5000.0

    # Read in trajectories for this rep
    con = sqlite3.connect(trajfile)
    data = pd.read_sql(
        'select * from freqs where repid == 42 and origin >= 45000 and origin <= 55000', con)
    con.close()
    data['scaled_time'] = pd.Series(
        (data.generation - 50000), dtype=np.float64) / 5000.0

    g = data.groupby(['esize', 'origin', 'pos'])

    # Filter groups into new pd.DF based on fixed vs. lost, then re-group
    fixations = g.filter(lambda x: x['freq'].max() == 1.0).groupby(
        ['esize', 'origin', 'pos'])

    # Filter any mutation that didn't live long enough or get to at least a certaing frequency, and did not fix
    losses = g.filter(lambda x: len(x['freq']) >= 5 and x['freq'].max(
    ) >= 0.01 and x['freq'].max() < 1.0).groupby(['esize', 'origin', 'pos'])
    ...
    axTOP.legend(loc='upper left', frameon=False)  # loc='upper left')
    ...
    for fix_i in FIXATIONS:
        ...
        if math.fabs(esize) > ghat and ENTRIES < 5:
            label = r'$\gamma = $' + \
                '{0:.2f}'.format(esize) + r', $o = $' + \
                '{0:0.4f}'.format((origin - 5e4) / 5e3)
            ENTRIES += 1
        axMID.plot(fix_i.scaled_time, fix_i.freq,  # color=fix_color,
                   ...
                   label=label)
    axMID.legend(loc="upper left", ncol=1, title=r'$\gamma = \mathrm{effect\ size}$,' +
    ...
    # Sort base on origin time, .min is again a hack
    LOSSES = sorted(LOSSES, key=lambda LI: LI.esize.min()
                    * LI.esize.min(), reverse=True)
    ENTRIES = 0
    for i in LOSSES:
        esize = i.esize.mean()
    ...
    axBOTTOM.legend(loc="upper left", ncol=1, title=r'$\gamma = \mathrm{effect\ size}$,' +
                    '\n' + r'$o = \mathrm{origination\ time}$', fontsize='small', frameon=False)
...
gs.tight_layout(fig)
plt.savefig("trajectoriesZoom.pdf", bbox_inches='tight')
