...
p = xyplot((50000-origin)/5e3~freq_at_shift|as.factor(mu)*as.factor(opt),data=data,
           mu = data$mu,
          par.settings=simpleTheme(pch=16,cex=0.5,alpha.points=0.5),
          auto.key=KEY, xlab="Mutation frequency at optimum shift",
          ylab=expression("Age of mutation (units of N generations"),
          scales=list(cex=1,alternating=F,x=list(rot=45)),
          strip=STRIP,
...
trellis.device(device="pdf",file="SoftSweepFreqAge.pdf",height=10,width=10)
print(p)
dev.off()
