...
for fi,i in zip(files,range(len(files))):
    ...
    x = pd.read_sql('select * from freqs where origin < 50000 and generation >= 50000',conn)
    g = x.groupby(['repid','origin','pos','esize'])['generation'].transform('max').reset_index(name='maxgen')
    x['maxgen']=g.maxgen
    x=x[x.generation==50000]
    print(opt,mu,x.esize.min(),x.esize.max())
    x.drop_duplicates(subset=['repid','origin','pos','generation'],inplace=True)
    conn.close()
    print(scipy.stats.mstats.mquantiles(x.esize,prob=[1e-2,0.5,1.-1e-2]))
    fig=plt.figure(figsize=(10,10))
    p=sns.jointplot('freq','esize',data=x,kind='hex',
            stat_func=None,xlim=(0,1),ylim=(-0.25,0.8),
            space=0)
    plt.subplots_adjust(right=0.9)
    cbaxes = inset_axes(p.ax_joint, width="45%", height="3%", loc=1) 
    cbar = plt.colorbar(cax=cbaxes, orientation='horizontal')
    cbar.ax.set_xticklabels(cbar.ax.get_xticklabels(),rotation=90)
    p.set_axis_labels('Frequency at optimum shift','Effect size ('+r'$\gamma$'+')')
    p.ax_joint.text(0.2,0.7,r'$z_o = $'+'{0:0.2f}'.format(opt) + '\n' +r'$\mu = $'+'{0:0.5f}'.format(mu))
    p.ax_joint.axhline(y=2*np.sqrt(2.0)*np.sqrt(mu),linestyle='dashed')
    p.ax_joint.axhline(y=-2*np.sqrt(2.0)*np.sqrt(mu),linestyle='dashed')
    p.ax_joint.axvline(x=0.05,linestyle='dashed')
