...
process_files <- function(d, param)
{
    ...
    for (i in d)
    {
        ...
        results = collect(dbt) %>%
            select(-vg,-zbar) %>%
            gather(locus,g,-generation,-repid) %>%
...
KEY=list(space="top",columns=3,
         ...
         just=0.5,
         text=key_text)
p = xyplot(mg~scaled_time|as.factor(mu)*as.factor(plarge),
           data=rank1,
           type='l',
...
p=xyplot(mg~scaled_time|as.factor(mu)*as.factor(plarge),data=gamma2_df,group=rank,type='l',
        auto.key=F,
        par.settings=simpleTheme(col=COLORS),
        lwd=3,
        scales=list(cex=1,alternating=F,x=list(rot=45)),
...
p=xyplot(mg~scaled_time|as.factor(mu)*as.factor(plarge),data=gamma_df,group=rank,type='l',
        auto.key=F,
        par.settings=simpleTheme(col=COLORS),
        lwd=3,
        scales=list(cex=1,alternating=F,x=list(rot=45)),
...
p=xyplot(mg~scaled_time|as.factor(mu)*as.factor(plarge),data=gauss_df,group=rank,type='l',
        auto.key=F,
        par.settings=simpleTheme(col=COLORS),
        lwd=3,
        scales=list(cex=1,alternating=F,x=list(rot=45)),
