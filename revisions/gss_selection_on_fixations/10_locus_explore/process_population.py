...
def process_pop(pop):
    ...
    for ut in utimes:
        ...
        for t in tv:
            ...
            s = np.where((sites['position'] >= l) & (sites['position'] < r))[0]
            for i in s:
                if sites['position'][i] not in fpos:
                    mut_idx = np.where(muts['site'] == i)[0]
                    assert len(mut_idx == 1), "Bad mutation table error"
                    sbelow = t.samples_below(muts['node'][mut_idx])
                    if len(sbelow) < 2*pop.N:  # filter out fixations
                        individuals = np.unique(sbelow // 2)
                        mg = amd['g'][mdidx[individuals]].mean()
                        gbar = amd['g'][mdidx].mean()
                        vg = amd['g'][mdidx].var()
                        ...
                        wbar = amd['w'][mdidx].mean()
                        esize = pop.mutations[muts['key'][mut_idx[0]]].s
                        data_notfixed.append(Datum(ut, sites['position'][i],
                                                   pop.mutations[muts['key']
                                                                 [mut_idx]].g,
                                                   np.nan, esize,
                                                   len(sbelow), mw, mg, wbar, gbar, vg))

    return data, data_notfixed
...
if __name__ == "__main__":
    ...
    pop = getpop(args)
    data, data_notfixed = process_pop(pop)
    df = pd.DataFrame(data, columns=Datum._fields)
    df_notfixed = pd.DataFrame(data_notfixed, columns=Datum._fields)
    with sqlite3.connect(args.outfile) as conn:
        df.to_sql('data', conn, if_exists="replace")
        df_notfixed.to_sql('data_notfixed', conn, if_exists='replace')
