...
loadaxes.append(plt.subplot(gs[3, 0]))
loadaxes.append(plt.subplot(gs[3, 1], sharex=loadaxes[0], sharey=loadaxes[0]))
loadaxes.append(plt.subplot(gs[3, 2], sharex=loadaxes[0], sharey=loadaxes[0]))
...
for i in range(len(datasets)):
    ...
    for g in grps:
        ...
        if Ng2 >= 1:
            ...
            loadaxes[i].plot(xdata, 1.0 - g[1].W,
                             linewidth=LINEWIDTH, ls=ls, alpha=alpha)
    ...
    saxes_inset[i].axhline(2./TWON, color="grey",
                           ls="dotted", linewidth=LINEWIDTH, alpha=0.3)
    loadaxes[i].axvline(tadapted, color="grey",
                        ls="dashed", linewidth=LINEWIDTH, alpha=0.3)
...
# Hide some axes!!!
for axes in freqaxes + distanceaxes + saxes:
    axes.get_xaxis().set_visible(False)

for axes in freqaxes[1:] + distanceaxes[1:] + saxes[1:]:
    axes.get_yaxis().set_visible(False)

for i, ax in enumerate(loadaxes):
    if i > 0:
...
for ax in [freqaxes[0], distanceaxes[0], saxes[0], loadaxes[0]]:
    for item in ax.get_yticklabels():
        item.set_fontsize(18)

fig.align_ylabels([i[0] for i in [freqaxes,
                                  distanceaxes,
                                  saxes, loadaxes]])

loadaxes[1].set_xlabel("Generations since optimum shift")
...
loadaxes[0].set_ylabel("Mean load")
...
loadaxes_notfixed.append(plt.subplot(gs[3, 0]))
loadaxes_notfixed.append(plt.subplot(
    gs[3, 1], sharex=loadaxes_notfixed[0], sharey=loadaxes_notfixed[0]))
loadaxes_notfixed.append(plt.subplot(
    gs[3, 2], sharex=loadaxes_notfixed[0], sharey=loadaxes_notfixed[0]))
# Plot notfixed
...
for i in range(len(datasets_notfixed)):
    ...
    for g in grps:
        ...
        if 5e3*esize*esize >= 1:
            ...
            saxes_notfixed[i].plot(
                xdata, s, linewidth=LINEWIDTH, ls=ls, alpha=alpha)
            loadaxes_notfixed[i].plot(xdata, 1.0 - g[1].W, linewidth=LINEWIDTH,
                                      ls=ls, alpha=alpha)
            # saxes_inset[i].plot(
            #     xdata, s, linewidth=LINEWIDTH, ls=ls, alpha=ALPHA)
    ...
    loadaxes_notfixed[i].axvline(tadapted, color="grey", alpha=0.3,
                                 ls="dashed", linewidth=LINEWIDTH)
    # saxes_inset[i].axvline(tadapted, color="grey",
    #                        ls="dashed", linewidth=LINEWIDTH)
    # saxes_inset[i].axhline(2./TWON, color="grey",
...
fig.align_ylabels([i[0] for i in [freqaxes_notfixed,
                                  distanceaxes_notfixed,
                                  saxes_notfixed, loadaxes_notfixed]])
...
for i, ax in enumerate(loadaxes_notfixed):
    if i > 0:
        ax.get_yaxis().set_visible(False)
    ax.set_xticks([0, 100, 200])
    for item in ax.get_xticklabels():
...
for ax in [freqaxes_notfixed[0], distanceaxes_notfixed[0],
           saxes_notfixed[0], loadaxes_notfixed[0]]:
    for item in ax.get_yticklabels():
        item.set_fontsize(18)

loadaxes_notfixed[1].set_xlabel("Generations since optimum shift")
...
loadaxes_notfixed[0].set_ylabel("Mean load")
