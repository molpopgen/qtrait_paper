...
def make_parser():
    ...
    parser.add_argument('--statsdb',type=str,default=None,help="File name for population stats database")
    parser.add_argument('--scandb',type=str,default=None,help="File name for genome scan database")
    return parser

DataPoint = namedtuple('DataPoint',['generation','VG','zbar','wbar'])
GenomeScanDataPoint = namedtuple('GenomeScanDataPoint',['generation','locus','window','D','Hprime'])
...
def get_summstats(pop, nsam, generation):
    ...
    for si, bi, locus_index in zip(s, pop.locus_boundaries, range(len(s))):
        ...
        for i in range(len(w)):
            ps = PolySIM(w[i])
            rv.append(GenomeScanDataPoint(generation,locus,i,ps.tajimasd(),ps.hprime()))
        locus += 1
    return rv
...
class Recorder(object):
    ...
    def __call__(self,pop):
        ...
        if c1 or c2 or c3 or c4: 
            ...
            self.data.append(DataPoint(pop.generation,traits['g'].var(),traits['g'].mean(),traits['w'].mean()))
            self.genome_scan_data.extend(get_summstats(pop,self.nsam,pop.generation))         
