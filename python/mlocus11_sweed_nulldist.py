...
SweedResult = collections.namedtuple('SweedEntry', ['LR', 'position', 'alpha'])
    ...
    for ts in msprime.simulate(NSAM, mutation_rate=THETA / 4.,
        ...
        for variant in ts.variants():
            data.append(SweedEntry(
                variant[0] * 1e6, len(np.where(variant[1] == 1)[0]), NSAM, 0))
...
if __name__ == "__main__":
    np.random.seed(SEED)
    seeds = [int(i) for i in np.random.choice(4000000000, 72, replace=False)]
    nreps = [int(1e6 / 72.)] * 72
