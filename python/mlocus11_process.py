...
def process_replicate(argtuple):
    ...
    vg_over_time = np.array([], dtype=[('repid', np.int32),
                                       ('generation', np.int32),
                                       ('vg', np.float)])
    gvalues = np.array([], dtype=[('repid', np.int32), ('generation',
                                                        np.int32), ('locus', np.int32), ('g', np.float)])
    gvalues_intermediate = np.array(
        [(-1, -1, -1, np.nan)] * 10, dtype=gvalues.dtype)
        ...
        while True:
            try:
                ...
                for i in range(len(gv_vg[0])):
                    gvalues_intermediate[i] = (
                        rec[0], rec[1].generation, i, gv_vg[0][i])
                # print("here3")
